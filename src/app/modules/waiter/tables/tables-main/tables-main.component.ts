...
export class TablesMainComponent implements OnInit, OnDestroy {
  ...
  turnOnNotifications() {
    this.notificationsSubscription = timer(0, 15000)
      .pipe(switchMap(() => this.orderService.getReadyTables()))
      .subscribe((resp) => {
        resp.forEach((table) => {
          this.tablesNotifications.push(table.id);
          if (!this.roomNotifications.some((e) => e == table.roomId)) {
            this.roomNotifications.push(table.roomId);
          }
        });
        if (this.tablesNotifications[0]) {
          this.selectNotification = true;
        } else {
      ...
      .subscribe((data) => {
        ...
        for (let i = 0; i < this.currentRoom.rows; i++) {
          for (let j = 0; j < this.currentRoom.columns; j++) {
            this.grid[i][j] =
              this.tables.find((table) => table.x == i && table.y == j) || null;
          }
        }
      });
  }
  ...
  checkReadyRoom(id) {
    if (this.roomNotifications.some((e) => e == id)) {
      return true;
    } else {
      return false;
    }
